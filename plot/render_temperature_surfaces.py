...
MOVIE_PATH = OUTPUT_DIR / "temperature_traced_surface_movie.mp4"
TRACED_SURFACES_PATH = PARAVIEW_DIR / "traced_temperature_surfaces.vtm"
...
MOVIE_FIELD_NAME = "Te"
MOVIE_FIELD_TITLE = "Electron temperature on traced field-line surfaces"
...
def _traced_lines_path(surface_name: str) -> Path:
    return PARAVIEW_DIR / f"temperature_field_lines_{surface_name}.vtm"
...
def main() -> None:
    print("--- Starting temperature traced-surface visualization ---")
    ...
    reference_field = field_data[MOVIE_FIELD_NAME]
    ...
    vmin = global_min
    vmax = float(np.nanpercentile(movie_field[-1], 90))
    if not np.isfinite(vmax) or vmax <= vmin:
        vmax = global_max if global_max > vmin else vmin + 1.0
    print(
    ...
    for frame_idx, t_idx in enumerate(tqdm(time_indices, desc="Rendering Frames")):
        ...
        for record in [outer_surface, middle_surface]:
            for segment_index, segment in enumerate(record["segments"]):
                temperature_surface = _sample_volume_on_surface(
                    movie_field[t_idx],
                    segment["r"],
                    segment["z"],
                    segment["phi"],
                ...
                frame_mesh[MOVIE_FIELD_NAME] = temperature_surface.ravel(order="F")
    ...
    plotter.close()
    print(f"--- Temperature surface movie complete: {MOVIE_PATH} ---")
    for line_path in line_paths:
        print(f"--- Temperature field lines exported to: {line_path} ---")
    print(f"--- Temperature surfaces exported to: {TRACED_SURFACES_PATH} ---")
